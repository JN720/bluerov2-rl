...
def generate_launch_description():
    ...
    with sl.group(if_arg='gui'):
        sl.gz_launch(sl.find('bluerov2_description', 'training_world.sdf'), "-r")
        
    with sl.group(unless_arg='gui'):
        sl.gz_launch(sl.find('bluerov2_description', 'training_world.sdf'), "-r -s")

    bridges = [GazeboBridge.clock(),
            #    GazeboBridge('/world/ocean/set_pose', '/set_pose', 'geometry_msgs/Pose', GazeboBridge.ros2gz),
